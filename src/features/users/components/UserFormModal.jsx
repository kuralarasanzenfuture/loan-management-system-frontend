...
// Helper function to format 10-digit phone numbers for display
...
export default function UserFormModal({
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      setForm({
        ...
        email: initialData.email || "",
        mobile: initialData.mobile
          ? initialData.mobile.replace(/\D/g, "").slice(0, 10)
          : "",
        password: "",
        role_id: initialData.role_id != null ? String(initialData.role_id) : "",
        status: initialData.status || "active",
  ...
  // Dedicated Handler for Mobile Number Input (digits only, max 10)
  const handleMobileChange = (e) => {
    const rawDigits = e.target.value.replace(/\D/g, "");
    if (rawDigits.length <= 10) {
      setForm((prev) => ({ ...prev, mobile: rawDigits }));
      setFieldErrors((prev) => ({ ...prev, mobile: null }));
    }
  };

  const handleLowerCaseChange = (field) => (e) => {
  const lowerValue = e.target.value.toLowerCase();
  setForm((prev) => ({ ...prev, [field]: lowerValue }));
  setFieldErrors((prev) => ({ ...prev, [field]: null }));
};
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="form-control">
            ...
            <input
              type="text"
              value={form.username}
              onChange={handleLowerCaseChange("username")}
              className={`input input-bordered input-sm rounded-lg ${
                fieldErrors.username ? "input-error" : ""
              }`}
              placeholder="e.g. user123"
          ...
          <div className="form-control">
            ...
            <input
              type="email"
              value={form.email}
              onChange={handleLowerCaseChange("email")}
              className={`input input-bordered input-sm rounded-lg ${
                fieldErrors.email ? "input-error" : ""
              }`}
              placeholder="e.g. example@gmail.com"
          ...
          <div className="form-control">
            ...
            <input
              type="tel"
              maxLength={14} // Allows formatted length e.g., (123) 456-7890
              value={formatPhoneNumber(form.mobile)}
              onChange={handleMobileChange}
              className={`input input-bordered input-sm rounded-lg ${
                fieldErrors.mobile ? "input-error" : ""
              }`}
