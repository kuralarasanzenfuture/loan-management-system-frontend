 ...
 */
export default function BankTransactionsPage({ bankId: bankIdProp, bankLabel }) {
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-base-content/50 mt-1">
            {bankLabel || (bankId ? `Bank Account #${bankId}` : "All bank transactions")}
          </p>
        </div>
        <button
          ...
          disabled={!bankId}
          title={!bankId ? "Select a bank account to record transactions" : undefined}
        >
          <Plus size={16} />
          New transaction
        </button>
      ...
      {!bankId && (
        <div className="alert alert-warning text-sm py-2 mb-4">
          <span>
            Please navigate to a specific bank account to view and record its transactions.
          </span>
        </div>
      )}
