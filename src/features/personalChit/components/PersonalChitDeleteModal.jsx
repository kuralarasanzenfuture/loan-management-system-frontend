...
import { AlertTriangle, Loader2, X, Wallet2 } from "lucide-react";
import { formatCurrency } from "../utils/chitHelpers.js";

export default function PersonalChitDeleteModal({
  open,
  chit,
  loading,
  error,
    ...
    <div className="modal modal-open">
      <div className="modal-box max-w-sm rounded-2xl">
        ...
        <div className="flex flex-col items-center text-center gap-3 pt-2">
          ...
          <h3 className="font-bold text-base">Delete this chit?</h3>
        </div>
        ...
        <div className="flex items-center gap-3 mt-4 p-3 rounded-xl border border-base-300 bg-base-200/40">
          ...
          <div className="min-w-0 text-left">
            <p className="font-semibold text-sm truncate">{chit.chit_name}</p>
            <p className="text-[11px] text-base-content/40 truncate">
              {chit.chit_no} · {formatCurrency(chit.chit_amount)}
            </p>
          </div>
        </div>

        <p className="text-sm text-base-content/60 text-center mt-4">
          This will permanently remove the chit and all its payment records.
