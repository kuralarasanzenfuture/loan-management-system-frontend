...
import ChitPaymentTable from "../components/ChitPaymentTable.jsx";
import ChitPaymentFormModal from "../components/ChitPaymentFormModal.jsx";
import RecordChitPaymentModal from "../components/RecordChitPaymentModal.jsx";
import ChitPaymentDeleteModal from "../components/ChitPaymentDeleteModal.jsx";
import {
  STATUS_STYLES,
  FREQUENCY_LABELS,
...
export default function PersonalChitViewPage() {
  ...
  // Schedule modal (create/edit installment_no, due_date, due_amount)
  const [scheduleModal, setScheduleModal] = useState(null); // null closed, {} new, {...row} edit
  const [scheduleSubmitting, setScheduleSubmitting] = useState(false);

  // Payment modal (record paid_amount, bit_benefit_amount, payment_mode, ...)
  // Kept entirely separate from scheduleModal — recording a payment is not
  // the same action as editing an installment's schedule, so it doesn't
  // reuse that modal or its state.
  const [payTarget, setPayTarget] = useState(null); // null closed, {...row} to pay
  ...
  // ---- Schedule: single installment create/edit ----
  const handleScheduleSubmit = async (formData) => {
    setScheduleSubmitting(true);
    try {
      const isEdit = Boolean(scheduleModal?.id);
      const { id: _paymentId, ...payload } = formData;
      const action = isEdit
        ? await dispatch(
            editPayment({ chitId: id, id: scheduleModal.id, formData: payload }),
          )
        : await dispatch(addPayment({ chitId: id, formData: payload }));
  ...
  // ---- Payment: record paid_amount / bit_benefit_amount against a row ----
  const handleRecordPayment = async (formData) => {
    if (!payTarget) return;
    setPaySubmitting(true);
    try {
      // Uses the same editPayment thunk as schedule edits (it's still a
      // PATCH on the same personal_chit_payments row), but the UI is
      // fully separate — see RecordChitPaymentModal. If your backend adds
      // a dedicated "record payment" endpoint later, swap this dispatch
      // for that thunk instead.
      const { id: _paymentId, ...payload } = formData;
      const action = await dispatch(
        editPayment({ chitId: id, id: payTarget.id, formData: payload }),
      );
      if (editPayment.fulfilled.match(action)) {
        setPayTarget(null);
        dispatch(fetchPersonalChitById(id)); // refresh chit-level totals
        dispatch(fetchPayments(id));
      }
    } finally {
  ...
  const totalBenefitAmount =
    payments?.reduce(
      (sum, p) => sum + (Number(p.bit_benefit_amount) || 0),
      0,
    ) || 0;
  const paidPercent =
    chit.chit_amount > 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="rounded-2xl border border-base-300 bg-base-100 px-5 py-4">
          <div className="text-xs text-base-content/50">Total Benefit</div>
          <div className="text-lg font-semibold leading-tight text-primary">
            {formatCurrency(totalBenefitAmount)}
          </div>
        </div>
        <div className="rounded-2xl border border-base-300 bg-base-100 px-5 py-4">
          <div className="text-xs text-base-content/50">Pending</div>
        ...
        <div className="rounded-2xl border border-base-300 bg-base-100 px-5 py-4">
          <div className="text-xs text-base-content/50">Received (Taken)</div>
          <div className="text-lg font-semibold leading-tight text-info">
            {chit.is_taken ? formatCurrency(chit.chit_received_amount) : "—"}
          </div>
        </div>
      </div>
      ...
      {/* Schedule: create/edit installment rows (no payment fields) */}
      <ChitPaymentFormModal
        open={Boolean(scheduleModal)}
        initialData={scheduleModal?.id ? scheduleModal : null}
        chit={chit}
        existingPayments={payments}
        suggestedInstallmentNo={nextInstallmentNo}
        loading={scheduleSubmitting}
        error={scheduleModal ? paymentsError : null}
        onClose={() => {
          setScheduleModal(null);
          dispatch(clearPaymentError());
        }}
        onSubmitSingle={handleScheduleSubmit}
        onSubmitBulk={handleBulkSubmit}
      />
      ...
      <RecordChitPaymentModal
        ...
        error={payTarget ? paymentsError : null}
        onClose={() => {
          setPayTarget(null);
          dispatch(clearPaymentError());
        }}
